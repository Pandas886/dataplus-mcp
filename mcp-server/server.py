#!/usr/bin/env python3
"""
Trino SSE MCP Server - Token Optimized Edition

This server provides Server-Sent Events (SSE) interface for Trino queries
...
try:
    from trino import TrinoError, TrinoQueryError
except ImportError:
    # Fallback to using generic exception
...
# Initialize Server
app_server = Server("trino-sse-optimized", version="1.0.0")
...
TRINO_CONFIG = {
    ...
    "catalog": os.getenv("TRINO_CATALOG", None),  # Optional default catalog
    "schema": os.getenv("TRINO_SCHEMA", None),    # Optional default schema
    # Connection settings for Docker environment
    "http_scheme": "http",
    "source": "mcp-trino-sse"
}
...
def format_compact_result(cursor, max_rows: Optional[int] = None) -> str:
    ...
    This format saves 60-70% tokens compared to JSON or Markdown tables.
    """
    ...
    rows = cursor.fetchmany(max_rows + 1)  # +1 to check if there are more rows
    ...
    for row in display_rows:
        ...
        for cell in row:
            if cell is None:
                ...
                cell_str = str(cell)
                # Truncate long cells to prevent context explosion
...
async def list_tools() -> List[Tool]:
    ...
    return [
        ...
        Tool(
            ...
            inputSchema={
                ...
                "properties": {
                    ...
                    "schema": {"type": "string", "description": "Schema name (optional, shows all schemas if not provided)"}
                },
                "required": ["catalog"]
            }
        ),
        Tool(
            name="sql_query",
            description="Execute READ-ONLY SQL queries. Returns token-optimized format. Use fully qualified names: catalog.schema.table",
            inputSchema={
                "type": "object",
                "properties": {
...
async def call_tool(name: str, arguments: dict) -> List[TextContent]:
    ...
    try:
        ...
        return [TextContent(type="text", text=f"Trino Error: {str(e)}")]
...
async def handle_sse(request: Request):
    ...
    # Set up SSE transport for bidirectional communication
    async def send(response):
        await request.send({"type": "http.response.start", "status": 200})
        await request.send({"type": "http.response.body", "body": response})

    async def receive():
        return await request.receive()

    async with app_server.connect(sse) as streams:
        await sse.handle_sse(request, streams[0], streams[1])

async def handle_messages(request: Request):
    """Handle POST messages for SSE."""
    # Forward the request to the SSE handler
    async def send(response):
        await request.send({"type": "http.response.start", "status": 200})
        await request.send({"type": "http.response.body", "body": response})
    ...
    await sse.handle_post_message(request, receive, send)
...
def main():
    ...
    logger.info(f"Starting Trino SSE MCP Server on {host}:{port}")
